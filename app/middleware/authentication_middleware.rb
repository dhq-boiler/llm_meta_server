...
class AuthenticationMiddleware
  ...
  def authenticate_api_request(request)
    ...
    # Fallback to external JWT authentication
  ...
  def verify_internal_jwt(request)
    ...
    begin
      decoded_token = JWT.decode(token, internal_jwt_secret, true, {
        algorithm: "HS256",
        verify_iat: true,
        verify_exp: true
      })
  ...
  def verify_external_jwt(request)
    token = extract_user_jwt_token(request)
    return { success: false, error: "No user token" } unless token

    begin
      decoded_token = JWT.decode(token, user_jwt_secret, true, {
        algorithm: "HS256",
        verify_iat: true,
        verify_exp: true
      })
      ...
      validate_user_jwt_payload(payload)

      user = User.find_by(id: payload["user_id"])
      return { success: false, error: "User not found", status: 404 } unless user

      unless user.email == payload["email"]
        return { success: false, error: "Token email does not match user email", status: 401 }
      end
  ...
  def validate_internal_jwt_payload(payload)
    ...
    expected_issuer = Rails.application.credentials.dig(:internal_jwt, :issuer) || "annotation_canvas"
    unless payload["iss"] == expected_issuer
      raise JWT::InvalidIssuerError, "Invalid issuer: #{payload["iss"]}"
    end

    expected_audience = Rails.application.credentials.dig(:internal_jwt, :audience) || "llm_api_call_meta_server"
    ...
    if payload["service"].present?
      allowed_services = %w[annotation_canvas llm_api_call_meta_server]
  ...
  def internal_jwt_secret
    Rails.application.credentials.dig(:internal_jwt, :secret) ||
  ...
  def internal_jwt_present?(request)
    request.params["internal_jwt"].present? ||
